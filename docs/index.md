...
This keeps a flat workspace where covers for APL+Win built-ins are prefixed with `_` since that character cannot begin a name in APL+Win. Add `-pre=_.` to instead put the covers into a namespace `_`. See `MIGRATE.APLPlusWin -??` for details.
